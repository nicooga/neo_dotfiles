...
class CardActivationViewController: ModalViewController {
    ...
    private var cardPendingActivation: Card? {
        ...
        guard let cardToBeActivatedUUID = Customer.current?.trackerCardUUID else {
            logError(
                "tried grabbing a card to activate and set a pin, but we didnt get the card uuid to activate from the card tracker"
            )
            return nil
        }
    ...
    private func setupConstraints() {
        NSLayoutConstraint.activate([
            ...
            expirationDateField.topAnchor.constraint(
                equalTo: verifyTitleLabel.bottomAnchor, constant: 40),
            expirationDateField.leadingAnchor.constraint(equalTo: view.leadingAnchor, constant: 20),
            expirationDateField.trailingAnchor.constraint(
    ...
    @objc private func whereCanIFindTapped() {
        ...
        let customViewComponent = AlertV2Component.custom(
            customViewInserted: customView, location: .top)
        let title = AlertV2Component.title("Where can I find this?".localize())
        let alertContent =
            "You can find the expiration date and CVC on the back of your Debit Card.".localize()
            .styled(with: .standardUnifiedApp)
        ...
        let action = AlertActionV2(
            alertButtonStyle: .defaultState, title: "Got it".uppercased().localize(), action: nil)
        ...
        cardPendingActivation?.activate(with: activationInfo)
            .send { [weak self] response in
                ...
                case .error(let error):
                    ...
                    if error.asNetworkError?.zeroClientMessage != nil
                        || (error.asNetworkError?.zeroHasMobileErrorParams ?? false)
                    {
                        return handleErrorAsToastOrBanner(
                            "This combination is incorrect.", error: error, dismissAction: nil,
                            completeDismissalActionWithBanner: true)
                    }
                case .success:

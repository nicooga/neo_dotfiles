    ...
    module Accounts
        ...
        class IsEligibleForAmfRefund
            ...
            def eligible_for_amf_refund?
                ...
                return false if has_disqualifying_transactions?
                true
            end
            ...
            def has_disqualifying_transactions?
                has_disqualifying_posted? || has_disqualifying_pending?
            end

            def has_disqualifying_posted?
                posted_transactions.any? { |t| DISQUALIFYING_POSTED_TYPES.include?(t.type_display) }
            end

            def has_disqualifying_pending?
                pending_transactions.any? { |t| DISQUALIFYING_PENDING_TYPES.include?(t.category_display) }
            end
            ...
            def pending_transactions
                @pending_transactions ||= Persistence::Transaction.pending!(account_id: input[:account].id)
            end
        end
    end
end
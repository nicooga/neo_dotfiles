require_dependency "action/accounts/is_eligible_for_full_amf_refund"

describe Action::Accounts::IsEligibleForFullAmfRefund do
  ...
  describe "when the account has an AMF product, has not paid any portion of the AMF and has no posted purchases or cash advances" do
    ...
    it "returns true" do
      expect(subject.is_eligible_for_full_amf_refund).to be true
    end
  end
  ...
  describe "when the account does not have an AMF product" do
    ...
    it "returns false" do
      expect(subject.is_eligible_for_full_amf_refund).to be false
    end
  end

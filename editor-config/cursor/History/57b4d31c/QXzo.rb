require_dependency "action/accounts/is_eligible_for_full_balance_refund"

describe Action::Accounts::IsEligibleForFullBalanceRefund do
  ...
  let(:unpaid_annual_charge_amount) { 1000 }
  ...
  describe "when the account has an AMF product, has not paid any portion of the AMF and has no posted purchases or cash advances" do
    it { is_expected.to be_success }

    it "returns true" do
      expect(subject.is_eligible_for_full_balance_refund).to be true
    end
  end
  ...
  describe "when the account does not have an AMF product" do
    ...
    it "returns false" do
      expect(subject.is_eligible_for_full_balance_refund).to be false
    end
  end

  describe "when the account paid any portion of the AMF" do
    let(:unpaid_annual_charge_amount) { rand(1..(annual_fee_cents - 1)) }

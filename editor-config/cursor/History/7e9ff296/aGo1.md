...
```ruby
def find_matching_account(fiserv_account_ids, transaction_type:, rejected_transaction_type: nil)
  transaction_types = transaction_type.responds_to(:each)
  ...
  fiserv_account_ids.each do |fid|
    ...
    next if action.output[:is_elegible_for_amf_refund]
    next unless posted_transactions_tally_by_type[transaction_type].to_i > 0
    next if rejected_transaction_type && posted_transactions_tally_by_type[rejected_transaction_type].to_i > 0
...
| TC5       |                   | `false`  |        |        |
| TC6       | 5159420100798980  | `false`  | `false`| ✅     |
| TC7       |                   | `true`   |        |        |
| TC8       |                   | TBD      |        |        |

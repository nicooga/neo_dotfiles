...
```ruby
def find_matching_account(fiserv_account_ids)
  fiserv_account_ids.for_each do |fid|
    acc, action = test_eligibility(fid); nil
    posted_transactions_tally_by_type = acc.send(:posted_transactions).map(&:type_display).tally


    if (
      action.outputs[:is_elegible_for_amf_refund])
      && (
        posted_transactions_tally_by_type["Cash"] > 0
        || posted_transactions_tally_by_type["Cash"] > 0
      )
    )
      return [acc, action]

...
```ruby
def find_matching_account(fiserv_account_ids, transaction_types:, rejected_transaction_type: nil)
  fiserv_account_ids.each do |fid|
    ...
    next unless transaction_types.any? do |t|
      posted_transactions_tally_by_type[t].to_i > 0
    end

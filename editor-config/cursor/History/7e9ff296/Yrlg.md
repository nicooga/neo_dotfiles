...
```ruby
def find_matching_account(fiserv_account_ids, transaction_type:, delay: 1, jitter: 0.5)
  fiserv_account_ids.each_with_index do |fid, idx|
    # Add delay with jitter to avoid overwhelming the service
    if idx > 0
      sleep_time = delay + rand * jitter
      puts "⏳ Waiting #{sleep_time.round(2)}s..."
      sleep(sleep_time)
    end
    ...
    paid_amf = action.send(:has_paid_amf?)
    txn_types = action.send(:posted_transactions).map(&:type_display).tally

    # Match: has AMF, unpaid AMF, and has the specific transaction type
    if has_amf && !paid_amf && txn_types[transaction_type].to_i > 0
      puts "✅ Found match: #{fid}"
      return [acc, action]
    end
  rescue => e
    puts "⚠️ #{fid} - Error: #{e.message}"
  end
